...
class SubscriptionCreationForm(forms.Form):
    plan_name = forms.CharField(required=True, label="Plan Name")
    plan_id = forms.CharField(required=True, label="Plan Id")
    monthly_charge = forms.IntegerField(required=True, label="Amount in cent",
        max_value=9999, widget=forms.TextInput(attrs={'size': '4'}))
